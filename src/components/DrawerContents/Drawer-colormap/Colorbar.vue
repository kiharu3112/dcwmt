  ...
  methods: {
    draw: function () {
      ...
      for (let i = 0; i < this.clrmap.length; i++) {
        this._drawRect(canvas, this.clrmap[i], rect_xpos, 0, rect_width, this.height);
        rect_xpos += rect_width;
      }

      // draw a triangle on the right.
      this._drawTriangle(canvas, this.clrmap[this.clrmap.length - 1], this.width, this.height, false);
    },

    _drawTriangle: (canvas: HTMLCanvasElement, color: ClrmapType, width: number, height: number, isLeft: boolean) => {
    ...
    _drawRect: (canvas: HTMLCanvasElement, color: ClrmapType, x: number, y: number, width: number, height: number) => {
      const context = canvas.getContext("2d") as CanvasRenderingContext2D;
      context.fillStyle = `rgb(${color.r}, ${color.g}, ${color.b})`;
